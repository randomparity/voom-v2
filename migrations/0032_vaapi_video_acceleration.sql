...
-- `-preset`) and the quality CHECK has to name four mutually exclusive columns,
...
-- Accelerator claims: 0031 closed `backend` to the two accelerators that
-- existed then. VAAPI is a Linux supervisor like NVIDIA, so it carries a
-- start identity; VideoToolbox remains the only backend without one.

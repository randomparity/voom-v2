...
| Unsupported or missing facts block visibly | `cargo test -p voom-plan defaults_best_blocks_instead_of_joining_executable_group`; attachment sources and attachment-target remux operations block until worker attachment preservation/removal is supported; remux fact blockers in `crates/voom-plan/src/planner_test.rs`. |

...
Sprint 7 uses a deterministic, testable token hasher abstraction at the
control-plane boundary so tests can assert behavior without depending on OS
randomness. Production token generation uses secure randomness. The token hash
format includes enough prefix/version information to allow a future hash
upgrade without rewriting every row.
...
node is retired, and `BAD_ARGS` for malformed CLI input. Do not add a public
error code unless implementation proves the existing vocabulary cannot express
the failure.
...
The use case verifies the node token and rejects retired nodes. On success, it
...
voom node heartbeat --node-id <id> --token <token>
...
```text
voom worker register --node-id <id> --token <token> --name <name> \

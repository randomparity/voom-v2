...
Sprint 7 injects `SharedRng` into `ControlPlane` so token generation is testable
without depending on OS randomness. Production control planes seed that shared
RNG from secure OS randomness, while tests inject `FrozenRng` through
`ControlPlane::open_with_pool_and_rng`. Token generation produces at least 256
bits of random token material. The plaintext token format is:
...
Verification must use constant-time comparison. Hashing and hint derivation are
deterministic functions; tests assert them from tokens produced with the
injected shared RNG.

...
| MP4 targets with MP4-incompatible non-video streams block visibly; an under-enumerated stream inventory blocks insufficient-facts | `cargo test -p voom-plan blocked_when_mp4_target_has_incompatible_subtitle blocked_insufficient_when_mp4_stream_inventory_underdescribed blocked_insufficient_when_mp4_target_and_streams_array_absent` | passed: 3 tests — incompatible subtitle blocks with the offending `ass` codec named; missing per-stream `codec_name` and an absent `streams` array both block `insufficient_snapshot_facts` |
...
| Output codec/container/dimension/pixel-format mismatch and source drift are visible and do not report success | `cargo test -p voom-control-plane transcode::dispatch` and `cargo test -p voom-ffmpeg-worker --test transcode_conformance` and `cargo test -p voom-control-plane transcode::tests::execute_rejects_worker_result_for_wrong_input_facts_before_commit` | passed: `dispatch_test::validate_output_facts_rejects_{width_over_cap,height_over_cap,pixel_format_mismatch,copied_video_disagreement}` reject mismatched output facts as `MalformedWorkerResult`; the worker conformance suite (17 tests) rejects a codec mismatch (`output_codec_mismatch_is_malformed_result`) and a bad container (`bad_payload_container_is_config_invalid`, `CONFIG_INVALID`); source drift surfaces as `ARTIFACT_CHECKSUM_MISMATCH` both at the worker (`wrong_expected_input_facts_is_checksum_mismatch`) and before commit in the control plane (`execute_rejects_worker_result_for_wrong_input_facts_before_commit`) |

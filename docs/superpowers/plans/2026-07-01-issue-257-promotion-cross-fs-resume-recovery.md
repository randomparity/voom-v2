...
- **Zero-warnings / commit-atomicity:** `just lint` (`cargo clippy --workspace --all-targets --all-features -- -D warnings`) runs as a prek pre-commit hook, and `--all-targets` builds the lib crate **without** `cfg(test)`. A private helper referenced only by sibling tests is dead code there and fails the gate. Therefore every new helper and the `move_terminal_artifact` rewrite that calls them land in **one commit** — do not commit a helper before its non-test caller exists. `tempfile` is already a dev-dependency of `voom-control-plane` (used by `mod_test.rs`), so the tests compile without a manifest change.
- Guardrails before the commit: `just fmt`, focused `cargo test -p voom-control-plane`, `just lint`. Before push: full `just ci`.
...
- Modify: `crates/voom-control-plane/src/workflow/coordinator/promotion.rs` — rewrite `move_terminal_artifact` and add five private free helpers (`resolve_existing_destination`, `copy_into_place`, `promotion_temp_path`, `remove_promoted_source`, `files_have_equal_contents`); add the `#[path]` sibling-test link at the bottom.
- Create: `crates/voom-control-plane/src/workflow/coordinator/promotion_test.rs` — sibling unit tests.

Both files change together for one responsibility (crash-safe, resumable promotion placement), so they land in a single commit.

---

### Task 1: Implement crash-safe resumable promotion placement

One TDD cycle, one commit. Write every failing test first, confirm the fix's regression test fails, implement all helpers plus the rewrite together (so nothing is dead code), confirm green, then commit once.

**Files:**
- Modify: `crates/voom-control-plane/src/workflow/coordinator/promotion.rs:103-146` (doc comment + `move_terminal_artifact` body) and imports; add helpers and the sibling-test link.
- Create: `crates/voom-control-plane/src/workflow/coordinator/promotion_test.rs`

**Interfaces (all private, same module — tests reach them via `use super::*`):**
- `async fn files_have_equal_contents(a: &Path, b: &Path) -> Result<bool, VoomError>` — `Ok(true)` iff identical bytes; size-first then chunked streaming compare; stat/open/read failure → `Err`.
...
- `async fn remove_promoted_source(current: &Path)` — best-effort `remove_file`; `tracing::warn!` on failure; never errors.
...
- `async fn resolve_existing_destination(current: &Path, dest: &Path, dest_meta: &std::fs::Metadata) -> Result<PathBuf, VoomError>` — source gone → `Ok(dest)`; source present + `dest` a regular file byte-equal to `current` → log, best-effort remove, `Ok(dest)`; else the `"promotion destination already exists"` error.
- `async fn move_terminal_artifact(current: &Path, dest: &Path) -> Result<PathBuf, VoomError>` — unchanged signature; pre-existing `dest` → `resolve_existing_destination`; else `rename` or `copy_into_place`.
...
- [ ] **Step 2: Write all failing tests in `promotion_test.rs`**
...
- [ ] **Step 3: Confirm the code does not yet compile / the fix's test fails**

Run: `cargo test -p voom-control-plane --lib promotion 2>&1 | tail -20`
Expected: FAIL — the helpers (`files_have_equal_contents`, `copy_into_place`, `resolve_existing_destination`) are not defined yet, so the test module does not compile. This is the pre-implementation red state (the `resumed_copy_recovers_and_removes_source` regression is what turns green after the rewrite).

- [ ] **Step 4: Add imports to `promotion.rs`**

At the top of `promotion.rs`, add:
...
- [ ] **Step 5: Add the five helpers to `promotion.rs`**
...
- [ ] **Step 6: Rewrite `move_terminal_artifact` (doc comment + body) at `promotion.rs:103-146`**
...
- [ ] **Step 7: Run the sibling unit tests — all green**

Run: `cargo test -p voom-control-plane --lib promotion`
Expected: PASS — all 11 tests, including `resumed_copy_recovers_and_removes_source`.

- [ ] **Step 8: Run the pre-existing collision regression tests**

Run: `cargo test -p voom-control-plane zero_phase_promotion_failure_preserves_seed_file_phases`
Expected: PASS (foreign `dest` = `b"existing"` differs from the artifact, still errors).

- [ ] **Step 9: fmt + lint (the exact prek gate)**

Run: `just fmt && just lint`
Expected: clean — no dead code (every helper is called by `move_terminal_artifact`/`resolve_existing_destination`), no pedantic warnings.

- [ ] **Step 10: Commit (single commit — helpers + rewrite + tests together)**
...
copy, both source and destination existed and the collision check failed the
run permanently on every resume. Recognise a destination that byte-equals the
still-present source as a resumed promotion and repoint; copy through a hidden
temp sibling then atomic rename so the destination is never partial; make source
removal best-effort.
...
Run: `just ci`
Expected: green — `fmt-check`, `lint`, `check-test-layout` (the `#[path]` link satisfies the sibling-test convention), `test`, `doc`, `deny`, `audit`.

- [ ] **Step 2: If any check fails, fix and re-run**

Most likely failure is a missing/mismatched `#[path = "promotion_test.rs"]` link (`check-test-layout`) — confirm it is at the bottom of `promotion.rs`. Fix, re-run `just ci`.
...
**Spec coverage:**
- D1 atomic copy via temp → `copy_into_place` + `promotion_temp_path` (Task 1 Step 5) + `copy_into_place_moves_bytes_and_cleans_up`.
- D2 content-verified recovery → `resolve_existing_destination` (Step 5) + `move_terminal_artifact` (Step 6); tests `resumed_copy_recovers`, `genuine_collision_same_size`, `genuine_collision_different_size`, `directory_destination`, `already_moved`; byte-equality via `files_have_equal_contents` + its four tests.
- D3 best-effort removal → `remove_promoted_source`, exercised by `copy_into_place`/recovery tests.
- Temp namespace disjointness → `promotion_temp_path` dotfile scheme + the temp-absent assertion in `copy_into_place_moves_bytes_and_cleans_up`.
- Preserve `"promotion destination already exists"` → Step 8 regression run + the three collision unit tests.
- Acceptance criteria (all 7 bullets) → mapped to the Task 1 tests.

**Placeholder scan:** No TBD/TODO/"handle edge cases"; every code step shows full code.

**Type consistency:** `files_have_equal_contents(&Path,&Path)->Result<bool,VoomError>`, `copy_into_place(&Path,&Path)->Result<(),VoomError>`, `promotion_temp_path(&Path)->Result<PathBuf,VoomError>`, `remove_promoted_source(&Path)`, `resolve_existing_destination(&Path,&Path,&std::fs::Metadata)->Result<PathBuf,VoomError>`, `move_terminal_artifact(&Path,&Path)->Result<PathBuf,VoomError>` — consistent across steps. `.voom-promote.<file_name>.partial` temp name identical in helper and test.

**Commit atomicity:** helpers, rewrite, and tests are one commit, so no intermediate state carries dead code past the `-D warnings` gate.

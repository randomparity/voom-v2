...
table once, directly in its final shape, generated by diffing its applied
result against the pre-squash 36-migration chain (schema and seed data
verified byte-identical). With no rebuild step anywhere in migration history,

...
Migration 0036 adds normalized `scan_sessions`, `scan_observation_batches`,
`scan_observations`, and `scan_reconciled_locations` tables. A session snapshots the root ID,
...
incarnation, availability, deadline, watermark, and in-flight commit locks. It records and
...
session `succeeded`, updates `library_roots.last_scan_session_id`, records reconciliation
evidence, appends the summary fact event, completes the idempotency response, and commits.
...
reconciliation ledger, rather than one event per location, is the durable per-location evidence;
this keeps an empty large-root completion bounded while preserving exact retired location IDs
and epochs for paginated inspection.
...
- **Emit one event for every retired location.** Rejected because a valid empty-root scan could
  create an unbounded event burst inside the completion transaction. The normalized
  reconciliation ledger is the detailed evidence and one session event records the aggregate
  fact.

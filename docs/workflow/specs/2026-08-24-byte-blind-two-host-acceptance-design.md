...
- `ControlChannelAccounting` is axum middleware on the production API router. It
  ...
  validates every non-empty body as JSON on an allowlisted production route, looks
  for each generated byte object's independent runtime marker in raw, hexadecimal,
  and alignment-safe base64 forms, and can discard exactly one committed response.

...
ADR 0013. It covers the SQLite schema through logical migration `0038` (physical version 3)
...
| `tickets.payload` | `ticket_payload.rs::parse_ticket` | `WorkflowTicketPayload`, `EffectiveTiming`, `ArtifactAccessDeclaration` and its target content structs (`StorageRootAccess`, `FileLocationAccess`, `ExistingArtifactAccess`, `PlannedArtifactAccess`, `ArtifactAccessEntry`) | The workflow envelope, timing fields, and the artifact-access declaration are strict. `rendered_payload` and `source_file` remain intentionally opaque inside that envelope. `ArtifactAccessDeclaration` routes its `Deserialize` through the same validating constructor as construction, so entry order, rights order, and multiplicity are wire contract too (ADR 0069). |
| `tickets.result` | compliance and workflow ticket-result decoders; `plan/expansion.rs::scanner_files` | `ExecuteExtractAudioOutputReport`, `ComplianceLegacyAudioExtractResult`, `PolicyVerificationTicketResult` | Published and historical typed result forms reject unknown fields. Recovery-only subpayloads remain opaque. A scan result's `files[]` is narrowed on the read side (ADR 0069): each entry must be an object carrying a non-zero `file_location_id`, and the former bare-string form is rejected, because a string names no location for a byte-touching child to declare. No production row is affected — no shipped plan carries a `ScanLibrary` node — but the accepted shape changed, so it is recorded here. |
...
| `artifact_commit_intents.expected_facts`, `artifact_commit_intents.receipt`, `artifact_commit_intents.supplemental_receipt` | `voom-store/src/repo/media/artifact_commit_intents.rs::row_to_intent` / `decode_receipt_column` | `CommitExpectedFacts`, `CommitReceipt` (`ApplyingReceipt`, `AppliedReceipt`, `MismatchedReceipt`, `OutcomeUnknownReceipt`) | Pinned expected facts and node-reported receipts reject unknown fields; the internally tagged receipt enum routes each variant through a strict wire struct and unknown vocabulary is a database error (ADR 0074). |
| `scan_observations.evidence_json` | `voom-store/src/repo/scan/sessions.rs::decode_observation_row`, batch route `typed_observation` in `voom-api/src/scan.rs` | `ScanObservationEvidence`, `ScanSidecarEvidence`, and `FileKeyFacts` (voom-core) | Agreed hash+probe identity evidence rejects unknown fields, non-`blake3:` digests, unknown sidecar roles, malformed blake3 hex, and the per-observation 64-sidecar / 64 KiB bounds (ADR 0077). NULL records existence without publishing identity. Decode failures are database errors. |

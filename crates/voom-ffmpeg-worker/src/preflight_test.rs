...
fn vaapi_ffmpeg_stub(dir: &Path, encode_arm: &str, decode_arm: &str) -> PathBuf {
    ...
    let body = format!(
        "#!/bin/sh\n\
         ...
         \x20 *-muxers*) cat <<'EOF'\n{ALL_MUXERS}EOF\n    ;;\n\
         \x20 *-init_hw_device*) echo '[VAAPI @ 0x1] VAAPI driver: Mesa Gallium driver 26.1.5 for AMD Radeon 8060S Graphics (radeonsi, strix_halo, ACO, DRM 3.64).' >&2 ;;\n\
         \x20 *-hwaccel*) {decode_arm} ;;\n\
         \x20 *hevc_vaapi*) {encode_arm} ;;\n\
         \x20 *) exit 2 ;;\n\
         esac\n"
    );

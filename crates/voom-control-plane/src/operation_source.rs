...
pub(crate) async fn select_local_source(
    ...
    let canonical_path = canonical_source_path(operation_label, &location.value).await?;
    Ok(SelectedSource {
        version,
        location,
        canonical_path,
...
async fn canonical_source_path(
    operation_label: &'static str,
    path: &str,
    ...
    match tokio::fs::symlink_metadata(path).await {
        ...
        Err(err) if err.kind() == ErrorKind::NotFound => {
            return Err(VoomError::ArtifactUnavailable(format!(
                "{operation_label} source artifact unavailable: {path}: {err}"
            )));
        }
        Err(err) => {
            return Err(VoomError::ArtifactUnavailable(format!(
                "cannot inspect {operation_label} source artifact {path}: {err}"
            )));
        }
    }

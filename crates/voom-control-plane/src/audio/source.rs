...
fn source_media_snapshot_id(operation_payload: &Value) -> Result<MediaSnapshotId, VoomError> {
    operation_payload
        ...
        .ok_or_else(|| {
            VoomError::Config(
                "audio operation payload requires source_media_snapshot_id".to_owned(),
            )
        })
}
    ...
    match tokio::fs::symlink_metadata(path).await {
        ...
        Err(err) if err.kind() == ErrorKind::NotFound => {
            return Err(VoomError::ArtifactUnavailable(format!(
                "audio source artifact unavailable: {path}: {err}"
            )));
        }
        Err(err) => {
            return Err(VoomError::ArtifactUnavailable(format!(
                "cannot inspect audio source artifact {path}: {err}"
            )));
        }
    }

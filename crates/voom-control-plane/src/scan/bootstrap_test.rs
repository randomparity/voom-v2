...
#[tokio::test]
async fn ensure_builtin_ffprobe_worker_reuses_existing_live_row() {
    ...
    assert_eq!(first.name, "builtin.ffprobe");
...
#[tokio::test]
async fn retired_builtin_ffprobe_worker_fails_loudly() {
    let (cp, _tmp) = cp_with_manual_clock(T0).await;

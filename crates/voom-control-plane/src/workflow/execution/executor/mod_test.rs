...
/// Seed one ready workflow ticket. `payload` decides whether it decodes.
async fn insert_ready_workflow_ticket(
    cp: &crate::ControlPlane,
    job_id: u64,
    ticket_id: u64,
    ...
    sqlx::query(
        "INSERT OR IGNORE INTO jobs (id, kind, state, priority, created_at, updated_at) \
         VALUES (?, 'workflow', 'open', 0, '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z')",
    )
    ...
    sqlx::query(
        "INSERT INTO tickets \
         ...
         (?, ?, 'synthetic.workflow.operation.hash_file', 'ready', ?, 1, 1, \
          '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z')",
    )
    .bind(i64::try_from(ticket_id).unwrap())
    .bind(i64::try_from(job_id).unwrap())
...
/// A payload pair: one that decodes, and one shaped like a pre-upgrade row —
/// a valid workflow envelope with no `declared_artifact_access`. The envelope
/// matters: `ready_workflow_tickets` selects on
/// `json_extract(payload, '$.workflow_id')`, so a payload mangled beyond
/// recognition would never be returned and would test nothing.
fn decodable_and_pre_upgrade_payloads(workflow_id: &str) -> (serde_json::Value, serde_json::Value) {
    ...
    .to_ticket_payload()
    .unwrap();
    let mut pre_upgrade = decodable.clone();
    // Unwrap, not discard: if the fixture ever stops carrying a declaration there
    // is nothing to remove and the "pre-upgrade" payload would be identical to the
    // decodable one, so both tests below would pass without testing anything.
    pre_upgrade
        .as_object_mut()
        .unwrap()
        .remove("declared_artifact_access")
        .unwrap();
    (decodable, pre_upgrade)
}

#[tokio::test]
async fn an_undecodable_ready_ticket_does_not_hold_back_its_well_formed_siblings() {
    let (cp, _db) = crate::cases::cp().await;
    let (decodable, pre_upgrade) = decodable_and_pre_upgrade_payloads("wf-74");
    insert_ready_workflow_ticket(&cp, 74, 941, &decodable).await;
    insert_ready_workflow_ticket(&cp, 74, 942, &pre_upgrade).await;
    ...
    // A decodable ticket remains, so the batch must not be fatal.
    let ready = executor
        .ready_workflow_tickets(JobId(74), "wf-74")
        .await
        .unwrap();

    assert_eq!(
        ready.iter().map(|ticket| ticket.id.0).collect::<Vec<_>>(),
        vec![941],
        "the well-formed ticket dispatches and the undecodable one is skipped"
    );
}

/// The run still has to stop. An undecodable ticket stays `ready` forever, so
/// `workflow_idle_state` keeps reporting `Ready` and the dispatch loop would spin
/// on an empty filtered batch. Raising once nothing dispatchable remains is what
/// keeps the skip above from turning a loud failure into a livelock.
#[tokio::test]
async fn a_batch_of_only_undecodable_tickets_fails_and_names_them() {
    let (cp, _db) = crate::cases::cp().await;
    let (_, pre_upgrade) = decodable_and_pre_upgrade_payloads("wf-75");
    insert_ready_workflow_ticket(&cp, 75, 951, &pre_upgrade).await;
    let executor = WorkflowExecutor::with_options(
        cp.clone(),
        WorkerRuntimeRegistry::new(),
        WorkflowExecutorOptions::for_tests(),
    ...
    // Nothing can dispatch, so the run must stop rather than spin.
    let error = executor
        .ready_workflow_tickets(JobId(75), "wf-75")
        .await
        .unwrap_err();

    let message = error.to_string();
    assert!(
        message.contains("951"),
        "the error names the ticket to drain: {message}"
    );
}
